...
# Page configuration
st.set_page_config(page_title="Communication Analysis & JSON Generator", page_icon="📋", layout="wide")
...
def generate_final_json(extracted_data: Dict[str, Any], missing_fields: List[str]) -> str:
    ...
    json_output = extracted_data.copy()
    ...
    json_output["_metadata"] = {
        ...
        "completion_percentage": round((len(extracted_data) / (len(extracted_data) + len(missing_fields))) * 100, 2) if (len(extracted_data) + len(missing_fields)) > 0 else 0
    }
...
def main():
    ...
    with st.sidebar:
        ...
        2. Dump your communications with the client into the Bot
        3. If there there are any gaps in the communication,the AI will prompt you for more information.
        4. If no relevant content can be provided, the AI will use its general knowledge to fill in any gaps it can
        5. AI will then exptract the relevant information and provide a response that can be added to the databases.
                    
        You can ask questions about the content or clarify requirements at any time, and the AI will provide responses based on the conversation history and the knowledge base.
        """)
        
        st.markdown("---")
        st.markdown("### 🔧 Features")
        st.markdown("""
        - **RAG-powered responses**: Uses your knowledge base content       
    ...
    # Chat input
    if prompt := st.chat_input("Please input client communication:"):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Searching knowledge base and generating response..."):
                    try:
                        ...
                        # Extract structured data, missing fields, and clarification questions
                        structured_data = extract_structured_data(answer)
                        missing_fields = extract_missing_fields(answer)
                        clarification_questions = extract_clarification_questions(answer)
                        
                        # Generate final JSON output
                        final_json = generate_final_json(structured_data, missing_fields)
                        
                        # Display extracted data and final JSON
                        with st.expander("📊 Extracted Data & JSON Output"):
